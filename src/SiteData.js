...
const SiteData = ({ children }) => {
    ...
    const getTotalAsset = () => {
        ...
        crypto_wallet.forEach(async (cw) => {
            ...
            setTotalAsset((prev) => prev + curr_price * Number.parseFloat(cw.balance));
        });
    };

    const API_ENDPOINT = "https://api.binance.com/api/v3";
    const getCurrentCryptoPrice = async (target) => {
        const response = await axios.get(`${API_ENDPOINT}/ticker/price?symbol=${target.toUpperCase()}USDT`);
